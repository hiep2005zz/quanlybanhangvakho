...
// Interceptor callback list cho lỗi 403 - không đủ quyền truy cập
type AccessDeniedHandler = (message: string) => void;
const accessDeniedHandlers: Set<AccessDeniedHandler> = new Set();

export function subscribeAccessDenied(handler: AccessDeniedHandler): () => void {
  accessDeniedHandlers.add(handler);
  return () => {
    accessDeniedHandlers.delete(handler);
  };
}

export function notifyAccessDenied(message: string = 'Bạn không có quyền truy cập chức năng này.') {
  accessDeniedHandlers.forEach((handler) => handler(message));
}
